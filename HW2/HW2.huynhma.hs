...
encodeList = foldr (\x t -> Node x End t) End
...
-- valueAt (h:t) (Node x y z)
--         | h == L = valueAt t y
--         | h == R = valueAt t z
-- Important that data constructors are capital in this case (L) and (R)
...
pathTo x (Node y l r)
      | x == y = Just []
      | (pathTo x l) /= Nothing = Just (L : fromJust (pathTo x l))
      | (pathTo x r) /= Nothing = Just (R : fromJust (pathTo x r))
      | otherwise = Nothing
